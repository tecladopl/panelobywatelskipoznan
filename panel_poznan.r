...
setwd('/home/kozun/PRACA/panele/losowania_final')
poznan <- read.csv(file = 'poznan_1.csv')
...
funkcja_oceniajaca <- function(v,b) {
  ...
  if (b) {
    ...
    write.csv(cbind(poznan$ID[v],poznan$Code[v]),'wynik_1.csv',row.names = FALSE)
  }
...
# Moglibysmy ewentualnie sprawdzac, czy elementy sie nie powtarzaja (to moze dac inny efekt - jestem ciekawy
# wyniku takiego eksperymentu).
# To jest wada uzycia GenSA - 
# gdybysmy pisali sami te procedure to moglibysmy juz "wewnetrznie" obslugiwac zbiory danych, a nie wektory.
